...
where t.xtype='U' order by t.name asc",MyConn);
        ...
        public static DataTable GetTableInfo(string TableName)
        {
            using (SqlConnection MyConn = new SqlConnection(DBConn))
            {
                SqlCommand MyCmd = new SqlCommand(@"select a.name 列名,a.length 长度,case when a.isnullable=1 then '是' when a.isnullable=0 then '否' end 能否为空,
case when a.IsIdentity=1 then '是' when a.IsIdentity=0 then '否' end 是否自增,
case when a.PK=1 then '是' when a.PK=0 then '否' end  是否主键,b.value as 描述,c.name as 类型 from 
(
